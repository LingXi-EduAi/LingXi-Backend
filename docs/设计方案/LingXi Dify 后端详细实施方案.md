...
* [x] BE\-05：冻结 LingXiEvent v1 协议并提供示例事件。

## 6\.2 MVP 主链路阶段

该阶段完成“创建任务—接收 Dify 流—持久化轨迹—推送前端—恢复会话”的闭环。每完成一项都应同步提供契约测试，避免前后端在事件字段上反复联调。

任务编号保持不变，实际按依赖顺序 `BE-08 -> BE-09 -> BE-07` 实施：先建立数据库结构，再让任务与事件持久化，最后在持久化事实之上实现续传、停止和重试。

* [x] BE\-06：基于 SseEmitter 实现流式代理、心跳、超时与断开清理。

* [x] BE\-07：实现停止、Last\-Event\-ID 续传和子任务重试。

* [x] BE\-08：执行数据库迁移并建立索引、唯一约束和回滚脚本。

* [x] BE\-09：持久化任务拆解、Agent 分配、节点状态和异常。
